...
- **Boot staging** — upstream brings everything up at once (full backlight from the first frame, WiFi at 19.5 dBm, BLE immediately after). Here the display **boots dark**: the backlight stays fully off until radios are up (`BootDiag` showed boots dying exactly where the capped-backlight and WiFi-connect loads overlapped), then jumps to the configured brightness. The CPU runs at 80 MHz through boot (160 after), WiFi connects at 8.5 dBm and restores full power after an IP is acquired, BLE starts 2.5 s after WiFi settles, and a short settle pause separates the screen-init peak from the WiFi burst. The **brownout detector is disabled for the boot window** and re-armed 5 s after boot completes — the resets *are* the detector firing on transient dips, so the boot rides through them while flash writes (settings, shot history, which all happen post-boot) keep protection. Expect a few dark seconds after power-on before the screen lights.
...
- **Timemore Black Mirror Dot scale (with battery)** — cherry-picked upstream's Dot support, but `esp-arduino-ble-scales` is **vendored in `lib/`** (base `67e0aa19` + the 7-line Dot battery decode) where upstream tracks the library unpinned. Later library commits require NimBLE 2.5 / esp-nimble-cpp, which this fork's base (NimBLE-Arduino 1.4.x) can't build against, and the battery decode landed after that switch — so no single upstream ref has both. Battery % surfaces as `sbat` in `evt:status`, in `/api/scales/info`, and as a pill on the Scales page (battery-only subset of upstream's scale-metadata polling; no flow-rate/unit/timer plumbing). See [lib/esp-arduino-ble-scales/VENDORED.md](../lib/esp-arduino-ble-scales/VENDORED.md).
