...
**BLE PID payload (10 fields):** `Kp,Ki,Kd,Kff,graceMs,kffEnabled,inletWaterTempC,pidFreezeEnabled,pidGraceEnabled,pidErrorAttenC` — older display firmware may send fewer fields; controller defaults missing tokens (attenuation 0 = off). See [`Settings.cpp`](../src/display/core/Settings.cpp) `buildPidBlePayload()`.
